...
export default function PostCard({ post, onTagClick }: PostCardProps) {
  ...
  return (
    <div
      ...
      className={cn(
        "group cursor-pointer rounded-lg border border-primary-100 bg-primary-50/50 p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md hover:bg-primary-50",
      )}
    >
      {/* Title */}
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {post.tags.map((tag) => (
            <button
              ...
              className="px-2.5 py-1 rounded-full bg-neutral-100 text-neutral-600 text-xs font-medium hover:bg-primary-50 hover:text-primary-700 transition-colors cursor-pointer"
            >
              {tag}
            </button>
          ))}
